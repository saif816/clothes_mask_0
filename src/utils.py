...
def compute_vae_encodings(image: torch.Tensor, vae: torch.nn.Module) -> torch.Tensor:
    ...
    print(f"VAE encoder input size: {pixel_values.size()}")
    with torch.no_grad():
        model_input = vae.encode(pixel_values).latent_dist.sample()
    model_input = model_input * vae.scaling_factor
    return model_input
...
async def load_image_from_url(session: aiohttp.ClientSession, url: str) -> PIL.Image.Image:
    """
    Downloads an image from a URL and loads it as a PIL Image.
    """
...
def format_error(e_type, e_value, e_traceback) -> str:
    """
    Function that formats error message for future output
    :param e_type: Error type
    :param e_value: Error value
    :param e_traceback: Error traceback
    ...
    error_message = "Exception type: %s, exception message: %s, stack trace: %s" % \
