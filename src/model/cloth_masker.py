...
class AutoMasker:
    ...
    def cloth_agnostic_mask(
        ...
        **kwargs
        ...
        legs_area = part_mask_of(["legs", "thighs", "feet"], densepose_mask, DENSE_INDEX_MAP) | \
            part_mask_of(
                ['Pants', 'Dress', 'Left-shoe', 'Right-shoe', 'Left-leg', 'Right-leg', 'Skirt'],
                schp_atr_mask, ATR_MAPPING
            ) | \
            part_mask_of(
                [
                    'Skirt', 'Left-shoe', 'Right-shoe', 'Left-leg',
                    'Right-leg', 'Skirt', 'Pants', 'Jumpsuits', 'Dress'
                ],
                schp_lip_mask,
                LIP_MAPPING
            )

        bbox = cv2.boundingRect(legs_area * 255)
        x,y,w,h = bbox
        legs_area = cv2.fillPoly(legs_area, [np.array([[x, y],[x + w, y], [x + w  * 1.2, y + h], [x * 0.8, y + h]], np.int32)], (255, 255, 255)) // 255
        legs_area = cv2.dilate(legs_area, dilate_kernel, iterations=2)

        # Image.fromarray(legs_area * 255).show()
        ...
        mask = Image.fromarray(mask_area * 255)
        return mask
    ...
    def __call__(
        ...
        assert mask_type in ['upper', 'lower', 'overall', 'inner', 'outer'], \
